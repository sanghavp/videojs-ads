...
import registry from './jsFlashBridgeRegistry';
// const VPAID_FLASH_HANDLER = 'vpaid_video_flash_handler';
// const ERROR = 'AdError';

export default class JSFlashBridge {
    constructor (el, flashURL, flashID, width, height, loadHandShake) {
        ...
        this._width = width;
        this._height = height;
        this._handlers = new utils.MultipleValuesRegistry();
        this._callbacks = new utils.SingleValueRegistry();
        this._uniqueMethodIdentifier = utils.unique(this._flashID);
        this._ready = false;
    ...
    removeCallbackByMethodName(suffix) {
        this._callbacks.filterKeys((key) => {
            return utils.stringEndsWith(key, suffix);
        }).forEach((key) => {
    ...
    setSize(newWidth, newHeight) {
        this._width = utils.isPositiveInt(newWidth, this._width);
        this._height = utils.isPositiveInt(newHeight, this._height);
